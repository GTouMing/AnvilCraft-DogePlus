    ...
    private void recompute(Network network) {
        ...
        for (Long2ObjectMap.Entry<GateNode> entry : network.nodes.long2ObjectEntrySet()) {
            ...
            for (Direction outputDir : Direction.values()) {
                int newSignal = gate.doge_plus$calculateOutput(level, blockPos, outputDir, inputMap);
        ...
        if (changed) {
            AnvilCraftDogePlus.LOGGER.info("recompute CHANGED network nodes={}", network.nodes.size());
            // 同步到持久化存储
            syncToData(network);
            // 通知邻居
